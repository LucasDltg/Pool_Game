...
class Pool:
    def __init__(self, size: list[int]):
        ...
        self.ball_diameter = self.height / 26
        self.InitBalls()
    ...
    def InitBalls(self) -> None:
        ...
        y = self.height / 2 - self.ball_diameter / 2  # middle of the table
        x = self.width / 4
        color_idx = 0
        ...
        for i in range(5):
            for j in range(i + 1):
                self.balls.append(ball.Ball(x, y + dy[color_idx] * self.ball_diameter, size=self.ball_diameter, color=colors[color_idx]))
                color_idx += 1
            x -= self.ball_diameter
        # white ball
        self.balls.append(ball.Ball(self.width * 3 / 4, self.height / 2 - self.ball_diameter / 2, 0, 0, size=self.ball_diameter, color=(255, 255, 255)))

    def SetBalls(self, balls: list[tuple[str, int, int]]) -> None:
        """
        Set the balls position and color
        :param balls:
        :return:
        """
        i = 0
        for i in range(len(balls)):
            #self.balls[i].x_pos = balls[i][1]
            #self.balls[i].y_pos = balls[i][2]
            self.balls[i].color = balls[i][0]

        for j in range(i, len(self.balls)):
            del self.balls[j]

    def Scale(self, previous_size, actual_size) -> None:
        self.ScalePoolTable(actual_size)
